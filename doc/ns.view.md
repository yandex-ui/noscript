...
`ctor` - это функция-конструтор. Обратите внимание, что он вызывается самым первым, до инициализации самого вида, т.о. в конструкторе еще не доступны некоторые свойства.
...
* `ns-view-init` — экземпляр вида создан, можно доопределить инициализацию
* `ns-view-destroyed` — экземпляр вида **будет** уничтожен (сразу после выполнения остальных обработчиков событий)
