...
- [Запрос данных модели с сервера](#Запрос-данных-модели-с-сервера)
...
- её можно запросить ещё раз (`canRetry()` возвращает `true`)

### Локальные модели

Локальная модель - модель, которая никогда не запрашивается на сервере.
Данные такой модели устанавливают вручную.
Чтобы модель стала локальной нужно переопределить метод `canRetry()`, к примеру, так:
